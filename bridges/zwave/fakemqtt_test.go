...
// its exported interface (see fakemqtt_test.go's use of mqtt.Client/mqtt.Token/mqtt.Message)
// rather than a real broker connection. It's the practical equivalent of
// bridges/esphome/fakenode_test.go's fakeESPHomeServer for this protocol: since mqttConn only
// ever talks to the network through the mqtt.Client interface, faking that interface exercises
// the exact same code path a real broker would, without needing to reimplement MQTT wire framing
// or stand up a real broker process.
//
// It has no automatic echo behavior: a test that wants WriteValue's echo-wait to succeed must
// call respond to arm the reply before triggering the write, mirroring how a real zwave-js-ui
// gateway only echoes back a value once it has actually accepted the write - a test that never
// arms a response exercises WriteValue's timeout path instead.
...
// synchronization to know the "broker" has already delivered its echo by the time WriteValue's
...
// duplicate, mirroring real paho's router.addRoute ("looks in the current list of routes to see
// if there is already a matching Route. If there is it replaces the current [handler]") - see
// mqttConn.handleConnect's doc comment, which relies on exactly this behavior to make
// re-subscribing on every reconnect safe. A fake that instead stacked duplicate routes would
// silently double-deliver every message to two live subscription handlers on a reconnect, which -
// for a WriteValue echo specifically - self-deadlocks (the second, redundant delivery falls
// through to onMessage on the same goroutine that's still holding the per-device lock
// WriteValue's own caller took out for the whole command). That's a real failure mode this bridge
// found via TestReconnect_RediscoversNodesAndPreservesLastNonZeroLevel while this method still
// appended blindly, not a hypothetical.
...
// single-level and '#' multi-level wildcards this bridge's own subscriptions use
// (<prefix>/# and _CLIENTS/ZWAVE_GATEWAY-<name>/api/+).
