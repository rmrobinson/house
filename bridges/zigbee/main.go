...
func main() {
	...
	if err := viper.UnmarshalKey("zigbee", &cfg); err != nil {
		logger.Fatal("unable to parse zigbee config", zap.Error(err))
	}

	svc := bridge.NewService(logger)

	zb, err := NewZigbeeBridge(logger, svc, cfg)
	if err != nil {
		logger.Fatal("unable to create zigbee bridge", zap.Error(err))
	}
